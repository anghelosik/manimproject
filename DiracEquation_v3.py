...
class DiracEquation_v3(Scene):
    def construct(self):
        # Titolo più compatto
        title = Text("L'Equazione di Dirac", font_size=36, color=BLUE)
        subtitle = Text("Meccanica Quantistica Relativistica", font_size=22, color=LIGHT_GRAY)
        ...
        # Equazione principale - ridotta ma leggibile
        dirac_eq = MathTex(
            r"(i\hbar\gamma^\mu\partial_\mu - mc)\psi = 0"
        )
        dirac_eq.scale(1.3)  # Leggermente più grande ma compatto
        dirac_eq.next_to(subtitle, DOWN, buff=0.8)
        self.play(Write(dirac_eq))
        self.wait(3)
        
        # Forma alternativa più esplicita
        alt_text = Text("Forma esplicita:", font_size=22, color=YELLOW)
        alt_text.next_to(dirac_eq, DOWN, buff=0.6)
        
        dirac_explicit = MathTex(
            r"i\hbar\frac{\partial\psi}{\partial t} = ",
            r"\left(-i\hbar c\vec{\alpha}\cdot\vec{\nabla} + mc^2\beta\right)\psi"
        )
        dirac_explicit.scale(1.0)  # Equazione di buona dimensione
        ...
        self.play(
            ...
            dirac_eq.animate.shift(UP * 0.8).scale(0.95)  # Leggermente più grande
        )
        
        # Box per i componenti
        components_title = Text("Componenti dell'equazione:", font_size=26, color=YELLOW)
        components_title.next_to(dirac_eq, DOWN, buff=0.6)
        self.play(Write(components_title))
        self.wait(1)
        
        # Componente 1: i (unità immaginaria)
        comp1_eq = MathTex("i", color=RED).scale(1.3)
        comp1_text = Text("unità immaginaria", font_size=20, color=WHITE)
        comp1_desc = Text("(caratteristica quantistica)", font_size=22, color=RED)
        ...
        comp1_group.to_edge(LEFT, buff=0.8)
        
        self.play(FadeIn(comp1_group))
        self.wait(2)
        
        # Componente 2: ℏ (costante di Planck ridotta)
        comp2_eq = MathTex(r"\hbar", color=GREEN).scale(1.3)
        comp2_text = Text("costante di Planck ridotta", font_size=20, color=WHITE)
        comp2_desc = MathTex(r"\hbar = \frac{h}{2\pi} \approx 1.055 \times 10^{-34}\text{ J·s}", 
                            font_size=32, color=GREEN)  # Testo verde
        ...
        comp3_eq = MathTex(r"\gamma^\mu", color=BLUE).scale(1.3)
        comp3_text = Text("matrici di Dirac (4×4)", font_size=20, color=WHITE)
        comp3_desc = Text("descrivono lo spin relativistico", font_size=20, color=BLUE)
        ...
        comp3_group.to_edge(LEFT, buff=0.8)
        
        self.play(FadeIn(comp3_group))
        self.wait(2)
        
        # Componente 4: ∂_μ (derivata parziale)
        comp4_eq = MathTex(r"\partial_\mu", color=ORANGE).scale(1.3)
        comp4_text = Text("derivata spazio-temporale", font_size=20, color=WHITE)
        comp4_desc = MathTex(r"\partial_\mu = \left(\frac{1}{c}\frac{\partial}{\partial t}, \vec{\nabla}\right)", 
                            font_size=32, color=ORANGE)  # Testo orange
        ...
        comp5_eq = MathTex("m", color=PURPLE).scale(1.3)
        comp5_text = Text("massa della particella", font_size=20, color=WHITE)
        comp5_desc = Text("(es: elettrone, positrone)", font_size=20, color=PURPLE)
        ...
        comp5_group.to_edge(LEFT, buff=0.8)
        
        self.play(FadeIn(comp5_group))
        self.wait(2)
        
        # Componente 6: c (velocità della luce)
        comp6_eq = MathTex("c", color=YELLOW).scale(1.3)
        comp6_text = Text("velocità della luce", font_size=20, color=WHITE)
        comp6_desc = MathTex(r"c \approx 3 \times 10^8 \text{ m/s}", 
                            font_size=28, color=YELLOW)  # Testo giallo
        ...
        comp7_eq = MathTex(r"\psi", color=TEAL).scale(1.3)
        comp7_text = Text("funzione d'onda (spinore)", font_size=20, color=WHITE)
        comp7_desc = Text("descrive lo stato della particella", font_size=20, color=TEAL)
        ...
        self.play(dirac_eq.animate.shift(DOWN * 0.3))
        
        meaning_title = Text("Significato fisico:", font_size=26, color=YELLOW)
        meaning_title.next_to(dirac_eq, DOWN, buff=0.6)
        self.play(Write(meaning_title))
        
        meaning1 = Text("• Descrive particelle con spin 1/2", font_size=22, color=WHITE)
        meaning1.next_to(meaning_title, DOWN, buff=0.3)
        meaning1.align_to(meaning_title, LEFT)
        self.play(FadeIn(meaning1, shift=RIGHT))
        self.wait(1.5)
        
        meaning2 = Text("• Unifica meccanica quantistica e relatività", font_size=22, color=WHITE)
        meaning2.next_to(meaning1, DOWN, buff=0.2, aligned_edge=LEFT)
        self.play(FadeIn(meaning2, shift=RIGHT))
        self.wait(1.5)
        
        meaning3 = Text("• Predice l'esistenza dell'antimateria", font_size=22, color=WHITE)
        meaning3.next_to(meaning2, DOWN, buff=0.2, aligned_edge=LEFT)
        self.play(FadeIn(meaning3, shift=RIGHT))
        self.wait(1.5)
        
        meaning4 = Text("• Base della elettrodinamica quantistica (QED)", font_size=22, color=WHITE)
        ...
        self.play(
            dirac_eq.animate.scale(1.15).move_to(ORIGIN),
            FadeOut(title_group)
        )
        ...
        final_box = Rectangle(
            ...
            color=BLUE,
            fill_color=BLUE,
            fill_opacity=0.05,  # Riempimento molto leggero
            stroke_width=3,
            stroke_color=BLUE_B
        )
        ...
        final_text = Text("Paul Dirac, 1928", font_size=22, color=GOLD)
        final_text.next_to(final_box, DOWN, buff=0.4)
